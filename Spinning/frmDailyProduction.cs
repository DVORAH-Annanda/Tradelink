    ...
    public partial class frmDailyProduction : Form
    {
        ...
        void SetUp(bool start)
        {
            ...
            if (start)
            {
                using (var context = new TTI2Entities())
                {
                    ...
                    var Card1Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate.Value == selectedDate && x.MachineNo_FK == 92).FirstOrDefault();
                    txtCard1Weight.Text = Card1Info != null ?  Card1Info.YarnProduction.ToString() : string.Empty;

                    var Card2Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK == 93).FirstOrDefault();
                    txtCard2Weight.Text = Card2Info != null ? Card2Info.YarnProduction.ToString() : string.Empty;

                    var Card3Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK == 94).FirstOrDefault();
                    txtCard3Weight.Text = Card3Info != null ? Card3Info.YarnProduction.ToString() : string.Empty;

                    var Card4Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK == 95).FirstOrDefault();
                    txtCard4Weight.Text = Card4Info != null ? Card4Info.YarnProduction.ToString() : string.Empty;

                    var RSB1Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK == 96).FirstOrDefault();
                    txtRSB1Weight.Text = RSB1Info != null ? RSB1Info.YarnProduction.ToString() : string.Empty;

                    var RSB2Info = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK == 97).FirstOrDefault();
        ...
        private void SaveYarnProduction(int machineNo, decimal yarnProduction)
        {
            ...
            if (selectedDate != null)
            {
                using (var context = new TTI2Entities())
                {
                    TLSPN_YarnProductionPerMachine yarnProductionInfo = context.TLSPN_YarnProductionPerMachine.Where(x => x.YarnProductionDate == selectedDate && x.MachineNo_FK== machineNo).FirstOrDefault();
