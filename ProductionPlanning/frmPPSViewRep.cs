...
namespace ProductionPlanning
{
    ...
    public partial class frmPPSViewRep : Form
    {
        ...
        public frmPPSViewRep(int RepNo, ProdQueryParameters PQP, ProgressBar progressBar)
        {
            InitializeComponent();
            _RepNo = RepNo;
            _ProdQParms = PQP;
        ...
        private void UpdateProgress(int value)
        {
            ...
            {
                _progressBar.Invoke(new Action<int>(UpdateProgress), value);
            }
            else
            {
            ...
            if (_RepNo == 11) // WIP Quick Look for Expedite
            {
                    ...
                    var customerMapping = new List<CustomerMapping>
                {
                    ...
                    using (var context = new TTI2Entities())
                    {
                        ...
                        var customerIds = new List<int> { 15, 16, 20, 23 };
                        ...
                        //            where customerIds.Contains(T1.TLCSVPO_Customer_FK) && !T1.TLCSVPO_Closeed && !T2.TLCUSTO_Closed && !T2.TLCUSTO_Picked
                        //            select T2).ToList();
                        ...
                        PODetail = (from T1 in context.TLCSV_PurchaseOrder
                                    join T2 in context.TLCSV_PuchaseOrderDetail on T1.TLCSVPO_Pk equals T2.TLCUSTO_PurchaseOrder_FK
                                    where customerIds.Contains(T1.TLCSVPO_Customer_FK) && !T1.TLCSVPO_Closeed && !T2.TLCUSTO_Closed 
                                    select T2).ToList();
                        ...
                        foreach (var Item in PPS)
                        {
                            ...
                            row.Colour = _Colours.FirstOrDefault(s => s.Col_Id == Item.TLREP_Colour_FK).Col_Display;
                            row.Size = _Sizes.FirstOrDefault(s => s.SI_id == Item.TLREP_Size_FK).SI_Description;
                            row.GPV001 = "0";
                            row.KZNVIC = "0";
                            row.TCT001 = "0";
                            row.TLWEC = "0";
                            ...
                            if (Orders != null && Orders.Any())
                            {
                                ...
                                // Split orders into customer-specific columns
                                foreach (var customer in customerMapping)
                                {
                                    var customerOrders = Orders.Where(o => o.TLCUSTO_Customer_FK == customer.Customer_FK);
                                    if (customerOrders.Any())
                                    {
                                        //var customerTotal = customerOrders.Sum(o => o.TLCUSTO_Qty);
                                        //row[customer.CustomerCode] = customerOrders.Sum(o => o.TLCUSTO_Qty).ToString();

                                        // Calculate nett outstanding for this customer
                                        var customerQtyOrdered = customerOrders.Sum(o => (int?)o.TLCUSTO_Qty) ?? 0;
                                        var customerQtyPicked = customerOrders.Sum(o => (int?)o.TLCUSTO_QtyPicked_ToDate) ?? 0;
                                        var customerNettOutstanding = customerQtyOrdered - customerQtyPicked;

                                        row[customer.CustomerCode] = customerNettOutstanding.ToString();
                                    }
                                    else
                                    {
                                        row[customer.CustomerCode] = "0";
                                    }
                                }
                            }
                            ...
                            if (ItemSOH != null && ItemSOH.Any())
                            {
                                ...
                                // Check if there are selected departments
                                if (_ProdQParms.Departments.Count > 0)
                                {
                                    // Reset the total boxed quantity to 0
                                    TotalBoxedQty = 0;

                                    // Sum up boxed quantities for the selected departments only
                                    foreach (var d in _ProdQParms.Departments)
                                    {
                                        // Sum quantities for the current department
                                        var deptTotal = ItemSOH.Where(x => x.TLSOH_CMT_FK == d.Dep_Id)
                                                               .Sum(x => (int?)x.TLSOH_BoxedQty) ?? 0;

                                        // Add to the overall total
                                        TotalBoxedQty += deptTotal;
                                    }
                                }
                            ...
                            // Parsing AvailableStock and TotalOrders as integers
                            int availableStock = 0;
                            int totalOrders = 0;

                            //if (!int.TryParse(row.AvailableStock, out availableStock))
                            // Check and parse AvailableStock
                            if (row.IsAvailableStockNull())
                            {
                                availableStock = 0; // Default to 0 if the value is DBNull
                            }
                            else if (!int.TryParse(row.AvailableStock, out availableStock))
                            {
                                availableStock = 0; // Default to 0 if parsing fails
                            }

                            // Check and parse TotalOrders
                            if (row.IsTotalOrdersNull())
                            {
                                totalOrders = 0; // Default to 0 if the value is DBNull
                            }
                            else if (!int.TryParse(row.TotalOrders, out totalOrders))
                            {
                                totalOrders = 0; // Default to 0 if parsing fails
                            }


                            // Calculating the difference and converting it back to string
                            int differenceStock = availableStock - totalOrders;
                            row.DifferenceStock = differenceStock.ToString();
                            ...
                            if (CMTPS.Count != 0)
                            {
                                ...
                                cmtPanels = Convert.ToInt32(Answer * 0.95);
                                row.CMTPanels = cmtPanels.ToString();
                            }
                            ...
                            if (CMTWIP.Count() != 0)
                            {
                                ...
                                if (WIP.Count != 0)
                                {
                                    cmtWIP = WIP.Sum(x => (int?)x.TLCUTSHRD_BundleQty - x.TLCUTSHRD_RejectQty) ?? 0;
                                    row.CMTWIP = cmtWIP.ToString();
                                }
                            }
                            ...
                            if (QueryC.Count() != 0)
                            {
                                cmtDespatch = QueryC.Sum(x => (int?)x.TLCMTWC_Qty) ?? 0;
                                row.CMTDespatch = cmtDespatch.ToString();
                            }


                            row.ROL = Item.TLREP_ReOrderLevel.ToString();

                            row.Expedite = (Item.TLREP_ReOrderLevel - differenceStock - cmtPanels - cmtWIP - cmtDespatch).ToString();
                ...
                catch (SqlException ex)
                {
                    ...
                    if (ex.Number == 1205)
                    {
                        MessageBox.Show("The system is currently overloaded. Please try running the report at another time.", "System Overloaded", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
