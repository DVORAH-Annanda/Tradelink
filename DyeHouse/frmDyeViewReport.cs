    ...
    public partial class frmDyeViewReport : Form
    {
        ...
        private void crystalReportViewer1_Load(object sender, EventArgs e)
        {
            ...
            else if (_RepNo == 13) 
            {
                ...
                using (var context = new TTI2Entities())
                {
                    var DCSoh = context.TLDYE_ConsumableSOH.ToList();
                    foreach (var item in DCSoh)
                    {
                        DataSet43.DataTable1Row nr = dataTable1.NewDataTable1Row();

                        var Cons = context.TLADM_ConsumablesDC.Find(item.DYCSH_Consumable_FK);
                        if (Cons != null)
                        {
                            ...
                            nr.StockOnHand = item.DYCSH_StockOnHand;
                            ...
                            nr.Location = context.TLADM_WhseStore.Find(item.DYCSH_WhseStore_FK).WhStore_Description;
            ...
            else if (_RepNo == 28)  //Shade Results straight after dyeing     
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    nr.NCReportTitle = "Report on Colour match results after dyeing for batches dyed from ";
            ...
            else if (_RepNo == 33)  //Stability check results after compacting   
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    var DyeBatchGroups = context.TLDYE_NonComplianceAnalysis.Where(x => x.TLDYEDC_Date >= _parms.FromDate && x.TLDYEDC_Date <= _parms.ToDate && x.TLDYEDC_NCStage == 5 & x.TLDYEDC_Pass).GroupBy(x => new { x.TLDYEDC_BatchNo, x.TLDYEDC_PieceNo_FK }).ToList();
            ...
            else if (_RepNo == 36)  // Fabric Sales Delivery Report    
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    var ExistingGroups = context.TLDYE_DyeBatchDetails.Where(x => x.DYEBO_Sold && x.DYEBO_DateSold >= _repOps.fromDate && x.DYEBO_DateSold <= _repOps.toDate).GroupBy(x=>x.DYEBO_TransactionNo).ToList();
