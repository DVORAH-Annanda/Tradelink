        ...
        private void frmDyeViewReport_Load(object sender, EventArgs e)
        {
            ...
            else if (_RepNo == 2)
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    var Existing = context.TLDYE_DyeOrderFabric.Find(_Pk);
                    if (Existing != null)
                    {
                        DataSet2.DataTable1Row nr = dataTable.NewDataTable1Row();

                        nr.DyeOrder = Existing.TLDYEF_DyeOrderNo;
                        nr.OrderDate = Existing.TLDYEF_OrderDate;
                        nr.Rating = Existing.TLDYEF_Yield;
                        nr.WeekNo = Existing.TLDYEF_DyeWeek;
                        nr.Demand = Existing.TLDYEF_Demand;

                        var Greige = _Greiges.FirstOrDefault(s => s.TLGreige_Id == Existing.TLDYEF_Greige_FK);
                        if (Greige != null)
                        {
                            nr.Quality = Greige.TLGreige_Description;

                            var fabwidth = context.TLADM_FabWidth.Find(Greige.TLGreige_FabricWidth_FK);
                            if (fabwidth != null)
                                nr.Wth = Convert.ToInt32(fabwidth.FW_Description);

                            var fabWeight = context.TLADM_FabricWeight.Find(Greige.TLGreige_FabricWeight_FK);
                            if (fabWeight != null)
                                nr.Disk = fabWeight.FWW_Calculation_Value;

                        }

                        nr.Customer = _Customers.FirstOrDefault(s => s.Cust_Pk == Existing.TLDYEF_Customer_FK).Cust_Description;

                        nr.Colour = _Colours.FirstOrDefault(s => s.Col_Id == Existing.TLDYEF_Colours_FK).Col_Display;
                        nr.Weight = Existing.TLDYEF_Demand;

                        DateTime dt = core.FirstDateOfWeek(Existing.TLDYEF_OrderDate.Year, Existing.TLDYEF_DyeWeek);
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    if (DyeBatch != null)
                    {
                        ...
                        else
                        {
                            ...
                            xnr.TotalKg = DyeBatch.DYEB_BatchKG;
            ...
            else if (_RepNo == 13)
            {
                ...
                using (var context = new TTI2Entities())
                {
                    var GroupDCSoh = context.TLDYE_ConsumableSOH.ToList();
                    foreach (var item in GroupDCSoh)
                    {
                        DataSet43.DataTable1Row nr = dataTable1.NewDataTable1Row();

                        var ConsPk = item.DYCSH_Consumable_FK;

                        var Cons = context.TLADM_ConsumablesDC.Find(ConsPk);
                        if (Cons != null)
                        {
                            nr.Code = Cons.ConsDC_Code;
                            nr.Description = Cons.ConsDC_Description;
                            nr.StdCost = Cons.ConsDC_StandardCost;
                            nr.ReOrderLevel = Cons.ConsDC_ReOrderLevel;
                            nr.SOHQuarantine = item.DYCSH_SOHQuar;
                            nr.SOHStore = item.DYCSH_StockOnHand;
                            nr.SOHDyeKitchen = item.DYCSH_SOHKitchen;
                            nr.StockTakeFreq = context.TLADM_StockTakeFreq.Find(Cons.ConsDC_StockTake_FK).STF_Description;
                            nr.PreferredSupplier = context.TLADM_Suppliers.Find(Cons.ConsDC_PreferedSupplier_FK).Sup_Description;
                            ...
                            if (DyeTran != null)
                            {
                                ...
                                if (Customer != null && first)
                                {
                                    ...
                                    foreach (var DBDetail in DBDetails)
                                    {
                                        ...
                                        tr2.ColourDescription = context.TLADM_Colours.Find(xDB.DYEB_Colour_FK).Col_Description;
                                        tr2.Quality = context.TLADM_Griege.Find(xDB.DYEB_Greige_FK).TLGreige_Description;
            ...
            else if (_RepNo == 18)  // Fabric Sales  
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    if (Record != null)
                    {
                        ...
                        var BDGrp = context.TLDYE_DyeBatchDetails.Where(x => x.DYEBO_TransactionNo == Record.TLDYET_TransactionNumber).OrderBy(x => x.DYEBD_DyeBatch_FK).GroupBy(x => x.DYEBD_DyeBatch_FK);
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    if (_repOps.FabricType) // ie Standard Stuff
                    {
                        DBBatches = (from T1 in DBBatches
                                     join T2 in context.TLKNI_GreigeProduction on T1.DYEBD_GreigeProduction_FK equals T2.GreigeP_Pk
            ...
            else if (_RepNo == 23)  //Dye and Chemical Consumption    
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    foreach (var Receipe in context.TLDYE_RecipeDefinition)
                    {
                        //1st thing find the colour in any of the Dye Batches 
                        //==========================================================
                        if (!Receipe.TLDYE_StandardReceipe)
                            continue;

                        DyeBatches = context.TLDYE_DyeBatch.Where(x => x.DYEB_BatchDate >= _repOps.fromDate && x.DYEB_BatchDate <= _repOps.toDate && x.DYEB_Colour_FK == Receipe.TLDYE_ColorChart_FK && x.DYEB_Allocated).ToList();
                        if (DyeBatches.Count <= 0)
                            continue;

                        var FirstTime = DyeBatches.Where(x => !(bool)x.DYEB_Reprocess).Sum(x => (decimal?)x.DYEB_BatchKG) ?? 0.00M;
                        var Reprocess = DyeBatches.Where(x => (bool)x.DYEB_Reprocess).Sum(x => (decimal?)x.DYEB_BatchKG) ?? 0.00M;

                        var ReceipeDefinitions = context.TLDYE_DefinitionDetails.Where(x => x.TLDYED_Receipe_FK == Receipe.TLDYE_DefinePk).ToList();
                        foreach (var ReceipeDefinition in ReceipeDefinitions)
                        {
                            DataSet24.DataTable2Row nr = dataTable2.NewDataTable2Row();
                            var Consumable = _Consumables.FirstOrDefault(s => s.ConsDC_Pk == ReceipeDefinition.TLDYED_Cosumables_FK);

                            nr.Pk = 1;
                            nr.Item = Receipe.TLDYE_DefineDescription;
                            nr.ItemDescription = Consumable.ConsDC_Description;
                            nr.Colour = _Colours.FirstOrDefault(s => s.Col_Id == Receipe.TLDYE_ColorChart_FK).Col_Display;
                            nr.BatchWeight = FirstTime + Reprocess;

                            if (!ReceipeDefinition.TLDYED_LiqCalc)
                            {
                                var Kgs = (ReceipeDefinition.TLDYED_MELFC / 100) * FirstTime;
                                nr.StdQtyKG = Kgs;

                                Kgs = (ReceipeDefinition.TLDYED_MELFC / 100) * Reprocess;
                                nr.ReprocessKG = Kgs;
                            ...
                            {
                                var Kgs = (ReceipeDefinition.TLDYED_MELFC / 1000) * FirstTime * ReceipeDefinition.TLDYED_LiqRatio;
                                nr.StdQtyKG = Kgs;

                                Kgs = (ReceipeDefinition.TLDYED_MELFC / 1000) * Reprocess * ReceipeDefinition.TLDYED_LiqRatio;
                                nr.ReprocessKG = Kgs;
                            }
                    ...
                    {
                        var ItemDC = context.TLADM_ConsumablesDC.Find(dr[0]);
                        if (ItemDC != null && !ItemDC.ConsDC_Discontinued)
                        {
                            DataSet24.DataTable2Row nr = dataTable2.NewDataTable2Row();
                            nr.Pk = 1;
                            nr.Item = dr[1].ToString();
                            nr.ItemDescription = dr[2].ToString();
                            nr.StdQtyKG = Decimal.Parse(dr[3].ToString());
                            nr.ReprocessKG = Decimal.Parse(dr[4].ToString());
                            nr.TotalKG = Decimal.Parse(dr[5].ToString());
                            nr.StandardCost = Decimal.Parse(dr[6].ToString());
                            nr.TotalCost = Decimal.Parse(dr[7].ToString());
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    foreach (var row in Batch)
                    {
                        ...
                        if (Existing != null)
                        {
                            ...
                            nr.Quality = _Quality.FirstOrDefault(s => s.TLGreige_Id == row.DYEB_Greige_FK).TLGreige_Description;
            ...
            else if (_RepNo == 25)  //Stock Take Sheet     
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    foreach (var row in consTable)
                    {
                        ...
                        nr.Store = context.TLADM_WhseStore.Find(row.ConsDC_StoreCode_FK).WhStore_Description;
            ...
            else if (_RepNo == 31)  //Dye Production Report _repOps
            {
                ...
                using (var context = new TTI2Entities())
                {
                    ...
                    DyeBatch = context.TLDYE_DyeBatch.Where(x => x.DYEB_OutProcess && x.DYEB_OutProcessDate >= _repOps.fromDate && x.DYEB_OutProcessDate <= _repOps.toDate && x.DYEB_OutProcess).ToList();
                    ...
                    else
                    {
                        if (!_repOps.FabricNotFinished)
                        {
                            ...
                            var result = from t in context.TLDYE_DyeBatch
                                         ...
                                         where x.DYEBO_QAApproved && x.DYEBO_ApprovalDate >= _repOps.fromDate && x.DYEBO_ApprovalDate <= _repOps.toDate && t.DYEB_OutProcess
                                         select t;
                        ...
                        else
                        {
                            ...
                            foreach (var row in result)
                            {
                                ...
                                nr.Quality = _Greige.FirstOrDefault(s => s.TLGreige_Id == row.t.DYEB_Greige_FK).TLGreige_Description;
                ...
                else
                {
                    if (!_repOps.FabricNotFinished)
                        ...
                        tr.Title = "Fabric Dyed Not Finished";
                }
            ...
            else if (_RepNo == 39) // DyeBatch Status Report
            {
                ...
                //========================================================
                IEnumerator ie = dbs.Section2.ReportObjects.GetEnumerator();
                while (ie.MoveNext())
                {
                    if (ie.Current != null && ie.Current.GetType().ToString().Equals("CrystalDecisions.CrystalReports.Engine.TextObject"))
                    {
            ...
            else if (_RepNo == 50)
            {
                ...
                NewRow.FromDate = _parms.FromDate;
                NewRow.ToDate = _parms.ToDate;
                NewRow.Title = "Greige Analysis";
                dataTable1.Rows.Add(NewRow);

                using ( var context = new TTI2Entities())
                {
                    var Records = context.TLDYE_NonComplianceAnalysis.Where(x => x.TLDYEDC_Date >= _parms.FromDate && x.TLDYEDC_Date <= _parms.ToDate && x.TLDYEDC_NCStage == 5 && x.TLDYEDC_Code_FK >= 12 && x.TLDYEDC_Code_FK <= 16).GroupBy(x=>x.TLDYEDC_BatchNo).ToList();
                    foreach (var Record in Records)
                    {
                        DataSet54.DataTable2Row NRow = dataTable2.NewDataTable2Row();
                        NRow.Pk = 1;
                        var DyeBatchPk = Record.FirstOrDefault().TLDYEDC_BatchNo;
                        var DBatch = context.TLDYE_DyeBatch.Find(DyeBatchPk);
                        if(DBatch != null)
                        {
                            NRow.BatchNo = DBatch.DYEB_BatchNo;
                            NRow.Quality = context.TLADM_Griege.Find(DBatch.DYEB_Greige_FK).TLGreige_Description;
                            var PiecePk = Record.FirstOrDefault().TLDYEDC_PieceNo_FK;
                            ...
                            {
                                if (Item.TLDYEDC_Code_FK == 12)
                                {
                                    NRow.LShrink = Item.TLDYEDC_Value;
                                }
                                else if (Item.TLDYEDC_Code_FK == 13)
                                {
                                    NRow.WShrink = Item.TLDYEDC_Value;

                                }
                                else if (Item.TLDYEDC_Code_FK == 14)
                                {
                                    NRow.Spirality = Item.TLDYEDC_Value;

                                }
                                else if (Item.TLDYEDC_Code_FK == 15)
                                {
                                    NRow.Courses = Item.TLDYEDC_Value;

                                }
                                else
                                {
                                    NRow.Wales = Item.TLDYEDC_Value;
                                }
                            }
                        }
